...
bfr_input_client::bfr_input_client(Authenticator *Auth,
...
// will be happy.
void bfr_input_client::process_init() {
...
void bfr_input_client::process_tstamp() {
...
unsigned int bfr_input_client::process_data() {
...
void bfr_input_client::process_quit() {
...
int bfr_input_client::min_reader(tmq_ref *tmqr, bool forcing) {
  if ( tmqr != first_tmqr ) return 0;
  int min = tmqr->Qrows_retired + tmqr->n_Qrows;
  std::list<bfr_output_client *>::iterator ocp;
  for ( ocp = all_readers.begin(); ocp != all_readers.end(); ++ocp ) {
    bfr_output_client *oc = *ocp;
...
void bfr_input_client::run_input_queue() {
  if (blocked_input) {
    // blocked_input = false;
    // flags |= Fl_Read;
...
bool bfr_input_client::run_output_queue(bool quitting) {
  bool blocked_output = false;
  std::list<bfr_output_client*>::iterator oci;
  for (lock(__FILE__,__LINE__), oci = all_readers.begin(), unlock();
       oci != all_readers.end();
       lock(__FILE__,__LINE__), ++oci, unlock()) {
    bfr_output_client *oc = *oci;
...
bool bfr_input_client::ready_to_quit() {
  while (!queue_empty()) {
    tmq_retire_check();
...
void bfr_output_client::transmit(bool quitting) {
  ...
  if ( data.tmqr == 0 ) {
    ...
    bfr_input_client::tm_gen->lock(__FILE__,__LINE__);
    if (bfr_input_client::tm_gen->next_tmqr(&data.tmqr)) {
      data.n_Qrows = 0;
    ...
    bfr_input_client::tm_gen->unlock();
  } else {
    ...
    while (tmqr) {
      ...
      if ( data.n_Qrows < tmqr->Qrows_retired ) {
        ...
        msg(MSG_DBG(0), "%s: Missed %d rows", iname, n_missed);
      }
      ...
      if ( nQrows_ready > 0 ) {
        if ( !blocked_input && (!srvr_has_shutdown()) &&
          ...
          hdr.s.hdr.tm_id = TMHDR_WORD;
          hdr.s.hdr.tm_type = bfr_input_client::tm_gen->output_tm_type;
          ...
          switch ( bfr_input_client::tm_gen->output_tm_type ) {
          ...
          iov[0].iov_base = &hdr.s.hdr;
          iov[0].iov_len  =
            bfr_input_client::tm_gen->tm_queue::nbDataHdr;
          Qrow_start = tmqr->Qrow + data.n_Qrows -
                          tmqr->Qrows_retired;
          if ( Qrow_start >= bfr_input_client::tm_gen->total_Qrows )
            Qrow_start -= bfr_input_client::tm_gen->total_Qrows;
          nl_assert(Qrow_start >= 0 &&
            Qrow_start < bfr_input_client::tm_gen->total_Qrows);
          nQ1 = nQrows_ready;
          nQ2 = Qrow_start + nQ1 -
                bfr_input_client::tm_gen->total_Qrows;
          if ( nQ2 > 0 ) {
            nQ1 -= nQ2;
            // SETIOV( &iov[2], row[0], nQ2 * nbQrow );
            iov[2].iov_base = bfr_input_client::tm_gen->row[0];
            iov[2].iov_len  =
              nQ2 * bfr_input_client::tm_gen->tm_queue::nbQrow;
            n_iov = 3;
          } else n_iov = 2;
          // SETIOV( &iov[1], row[Qrow_start],
                      // nQ1 * nbQrow );
          iov[1].iov_base = bfr_input_client::tm_gen->row[Qrow_start];
          iov[1].iov_len  =
            nQ1 * bfr_input_client::tm_gen->tm_queue::nbQrow;
      ...
      } else if (bfr_input_client::tm_gen->next_tmqr(&tmqr)) {
        nl_assert(tmqr);
...
bool bfr_output_client::iwritten(int ntr) {
  if (obuf_empty()) {
    ...
    output.ready = true;
    if (blocked_input && bfr_input_client::tm_gen) {
      msg(MSG_DBG(0), "%s: Calling run_input_queue", iname);
      bfr_input_client::tm_gen->run_input_queue();
    }
  }
  return false;
}

bool bfr_output_client::protocol_timeout() {
  if (quit_sent) {
...
bool bfr::ready_to_quit() {
  ...
  if (bfr_input_client::tm_gen) {
    return bfr_input_client::tm_gen->ready_to_quit();
  }
  return true;
}
