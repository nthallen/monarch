...
memo_socket *new_memo_socket(Authenticator *Auth, SubService *SS) {
  memo_socket *memo = new memo_socket(Auth, SS);
  return memo;
}
...
/* Begin transfer from memo_notes.cc (2019 Jan 15) */	
	oui_init_options(argc, argv);
	// Initialize stuff
	Server memo("memo", 500);
	memo.Subs.add_subservice(new SubService("memo", (socket_clone_t)new_memo_socket, (void*)0));
	memo.Start(Srv_Unix); // Change to Srv_Both when TCP is implemented
	return 0;
/* End transfer from memo_notes.cc */
