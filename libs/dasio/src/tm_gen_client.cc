...
/** tm_gen_client.h Framework for 
*/

tm_gen_client::tm_gen_client() : public DAS_IO::Serverside_client {
  dispatch = NULL;
}

tm_gen_client::~tm_gen_client() {
  if ( dispatch != NULL )
    detach();
}

void tm_gen_client::attach(tm_gen_dispatch *disp) {
  // assert( disp != NULL );
  dispatch = disp;
  dispatch->client_add(this);
}

void tm_gen_client::detach() {
...
int tm_gen_dispatch::all_closed() {
  int ready = 1;
  int not_ready = 0;
  std::list<tm_gen_client *>::iterator pos;
...
void tm_gen_dispatch::client_add(tm_gen_client *clt) {
  //assert(clt != NULL);
  clients.push_back(clt);
}

void tm_gen_dispatch::client_rm(tm_gen_client *clt) {
  //assert(clt != NULL);
  clients.remove(clt);
}
