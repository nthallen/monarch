...
static char *yy_text;
static int32_t yy_val;

static void synt_err(const char *txt) {
  msg( 2, "%s:%d %s", yy_filename, yy_lineno, txt );
}
...
static int buffer_char( char c ) {
  if ( lexbufpos >= lexbufsize ) {
    ...
    yy_text = (char*)realloc( yy_text, lexbufsize );
    if ( yy_text == 0 ) {
...
static void yyunlex( int token ) {
  if ( yyunlexed )
    msg( 4, "Cannot yyunlex two tokens" );
  yyunlexed = token;
}
  ...
  for (;;) {  
    switch ( c = getc(fp) ) {
      ...
      if ( isspace( c ) ) break;
        ...
        ungetc( c, fp );
        END_BUFFER;
        if ( strcasecmp( yy_text, "validate" ) == 0 )
          return KW_VALIDATE;
        else if ( strcasecmp( yy_text, "and" ) == 0 )
          return KW_AND;
        else if ( strcasecmp( yy_text, "or" ) == 0 )
          return KW_OR;
        else if ( strcasecmp( yy_text, "hold" ) == 0 )
          return KW_HOLD;
        else if ( strcasecmp( yy_text, "else" ) == 0 )
          return KW_ELSE;
        else if ( strcasecmp( yy_text, "resume" ) == 0 )
          return KW_RESUME;
        return TK_NAME;
      } else {
...
static int tma_strdup( command_t **ptr, const char *str, command_t *next ) {
  command_t *cmdl;
  cmdl = (command_t*)malloc( sizeof(command_t) );
  if ( cmdl != 0 ) {
    cmdl->cmd = strdup( str );
    if ( cmdl->cmd != 0 ) {
      cmdl->next = next;
  ...
  switch ( token ) {
    case TK_TMCCMD:
      { int oldresp = set_response( 1 );
        int rv;
        if ( yy_text[1] == '_' )
          rv = ci_sendcmd(Cmd_Test, yy_text+2);
        else rv = ci_sendcmd(Cmd_Test, yy_text+1);
...
static int read_tmafile( tma_ifile *spec, FILE *fp ) {
  ...
  spec->cmds = (tma_state*)malloc( max_cmds * sizeof( tma_state ) );
  if ( spec->cmds == 0 ) {
    msg( 2, "Out of memory reading tma file" );
    return 1;
  }
  sv = find_slurp( spec->statename );
  if ( sv == NULL ) {
    msg( 2, "State name '%s' not found in slurp_vals",
      spec->statename );
    free_tmacmds( spec );
    return 1;
  }
  ...
  for (;;) {
    ...
    while ( cmdl != NULL ) {
      ...
      if ( ++n_cmds == max_cmds ) {
        ...
        max_cmds *= 2;
        ncmds = (tma_state*)
          realloc( spec->cmds, max_cmds * sizeof( tma_state ) );
        if ( ncmds == 0 ) {
          if ( cmdl->cmd != 0 ) {
            free(cmdl->cmd);
...
void tma_read_file( tma_ifile *ifilespec ) {
  ...
  if ( modtime != ifilespec->modtime && ifilespec->cmds == 0 ) {
    ...
    if ( fp == 0 ) {
      msg( 1, "Unable to open algo file %s for state %s", 
            ifilespec->filename, ifilespec->statename );
    } else {
      msg( -2, "Reading algo file %s", ifilespec->filename );
