%{
  ...
  static void append_command(struct cmdlst *cl, struct cmddef *cmd) {
	if (cl->first == NULL) cl->first = cl->last = cmd;
	  ...
	  /* msg(2, "TMC Command must follow all commands for time"); */
...
state_def : KW_STATE state_name_def opt_state_file opt_nolog state_cmds {
		msg(PARSE_DEBUG, "State Definition %s", $2);
...
state_name_def : TK_NAME {
		  $$ = new_state_name($1);
		  msg( PARSE_DEBUG, "state_name_def: %s", $1 );
		}
	;
...
time_spec : { $$ = last_command_time; }
	| time {
		if ($1 < last_command_time)
		  msg(2, "Command Time preceeds previous command");
		$$ = last_command_time = $1;
	  }
...
timed_command : TK_COMMAND {
		msg(PARSE_DEBUG, "Read command \"%s\"", $1);
		check_command($1);
		$$ = new_command( CMDTYPE_CMD, $1, NULL );
	  }
	| TK_QSTRING ';' {
		msg(PARSE_DEBUG, "Qstring %s", $1 );
...
hold_clause : hold_cond ';' {
		msg(PARSE_DEBUG, "Hold w/o timeout" );
		$$ = $1;
		$$->timeout = -1;
		$$->else_stat = NULL;
	  }
	| hold_cond KW_OR time else_clause {
		msg(PARSE_DEBUG, "Hold w/ timeout %d", $3 );
		$$ = $1;
		$$->timeout = $3;
		$$->else_stat = $4;
	  }
...
hold_cond : { $$ = new_command( CMDTYPE_HOLDV, NULL, NULL ); }
	| KW_AND KW_VALIDATE TK_NAME {
		msg(PARSE_DEBUG, "(hold_cond Hold and Validate %s)", $3 );
		$$ = new_command( CMDTYPE_HOLDV, $3, NULL );
	  }
	| KW_UNTIL opt_valid TK_PARENSTAT {
		msg(PARSE_DEBUG, "(hold_cond Hold until %d %s)", $2, $3 );
