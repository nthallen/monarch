...
/**
  tm_gen_dispatch_client is a virtual base class for clients of tm_gen_dispatch. The main service provided is quit coordination.
...
class tm_gen_dispatch {
  public:
    ...
    tm_gen_dispatch();
    ~tm_gen_dispatch();
    void client_add(tm_gen_dispatch_client *clt);
    void client_rm(tm_gen_dispatch_client *clt);
    void Loop();
    void ready_to_quit();
  private:
    int quit_received;
    ...
    std::list<tm_gen_dispatch_client *> clients;
};
